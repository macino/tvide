                ...
                void (&onError)(const char *) ) noexcept
{
    ...
    if (clientPid == 0)
    {
        ...
        // Check if TVDESK_EXEC is set - if so, run that command in the shell
        char *execCmd = getenv("TVDESK_EXEC");
        char *shell = getenv("SHELL");
        
        if (execCmd && execCmd[0] != '\0') {
            // Execute command via shell: shell -c "command"
            char *args[] = {shell, (char*)"-c", execCmd, nullptr};
            execvp(shell, args);
        } else {
